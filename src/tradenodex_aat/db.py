    ...
    with connect() as conn:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS bots (
              ...
              dry_run INTEGER NOT NULL,
              max_position_usdt REAL NOT NULL,
              risk_per_tick_usdt REAL NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS positions (
              ...
              unrealized_pnl REAL,
              updated_at TEXT NOT NULL,
              UNIQUE(exchange, symbol, side, account_id)
            );
...
def create_account(data: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        'dry_run': int(data.get('dry_run', True)),
        'api_key_encrypted': encrypt_secret(data.get('api_key')),
        'api_secret_encrypted': encrypt_secret(data.get('api_secret')),
        'api_passphrase_encrypted': encrypt_secret(data.get('api_passphrase')),
        'created_at': now,
        'updated_at': now,
    }
    with connect() as conn:
...
def public_account(row: dict[str, Any] | sqlite3.Row) -> dict[str, Any]:
    ...
    return {
        'id': item['id'], 'name': item['name'], 'exchange': item['exchange'], 'environment': item['environment'],
        'base_currency': item['base_currency'], 'dry_run': bool(item['dry_run']),
        'api_key_masked': mask_secret(item.get('api_key_encrypted')), 'has_secret': bool(item.get('api_secret_encrypted')),
...
def create_bot(data: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        'id': data.get('id') or str(uuid4()),
        'name': data['name'], 'type': data['type'], 'exchange': data['exchange'],
        'symbols_json': json.dumps(data.get('symbols') or ['BTCUSDT']),
        'quote_currency': data.get('quote_currency', 'USDT'), 'status': data.get('status', 'DRAFT'),
        'dry_run': int(data.get('dry_run', True)), 'max_position_usdt': float(data.get('max_position_usdt', 100)),
        'risk_per_tick_usdt': float(data.get('risk_per_tick_usdt', 5)), 'grid_levels': int(data.get('grid_levels', 8)),
        'take_profit_pct': float(data.get('take_profit_pct', 0.8)), 'stop_loss_pct': float(data.get('stop_loss_pct', 3.0)),
        'account_id': data.get('account_id'), 'created_at': now, 'updated_at': now
    }
    with connect() as conn:
