...
async def reconcile_account(account: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {'account_id': account['id'], 'positions': positions, 'open_orders': open_orders, 'balances': balances}
    add_log('Account reconciliation completed', detail={'account_id': account['id'], 'exchange': account['exchange'], 'positions': len(positions), 'open_orders': len(open_orders), 'balances': len(balances)})
    return result
